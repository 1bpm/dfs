...
function perform(theRoles, event, duration) {
    ...
    } else if (duration instanceof Trigger) {
        {
            progressBar:false
        }
        ;
...
function baseTrigger(type, role, arg1, arg2) {
    ...
    var Trigger = function (data) {
        ...
        data.name = (data.name || "tr" + (Object.keys(triggers).length + 1));
        ...
        if (!_system.live) {
            role.triggers[data.name] = thisTrigger;
            triggers[data.name]=thisTrigger;
        }
    };
