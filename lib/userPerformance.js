...
var dftool = {
    ...
    gaussianRand: function (mean, std) {
        if (mean === undefined || std === undefined) {
            throw "gaussianRand needs 2 arguments (mean, standard deviation)";
        }
...
this.runScoreTest = function (score, roleName) {
    _system.live = false;
    var maxTime = 0;
    var score = "try{" + score + "}catch(e){throw e.stack;}";
    ...
    if (roleName !== "perform") {
        _system.Future.task(function () {
            try {
                ...
                var starter = new TextEvent("Get ready", {
                    throb: false,
                    throbNext: true,
                    progressBar: true,
                    showNextEvent: true,
...
this.complete = function () {
    ...
    if (adm) {
        ...
        adm.view("loading", {title: "Performance complete", text: "Please wait"});
        adm.emit("performanceComplete", {});
        setTimeout(function () {
            //
        }, 3000);
    }
...
var Observer = function (theClient) {
    ...
    this.perform = function (roleName,current, duration, next) {
        client.emit("observeEvent", {role:roleName,id: current, mini: next, duration: duration});var stamp = new Date().getTime();
