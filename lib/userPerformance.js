...
var Role = function (roleName, data) {
    ...
    if (!name) name="Role "+(Object.keys(roles).length);
    ...
    this.addPerformer = function (performer) {
        ...
        performer.emit("view", {state: "performanceReady", data: {events: events, role: {name:name,totalEvents:totalEvents}, performance: performance}});
        ready = false;
    };
...
var Event = function (data) {
    ...
    var content;
    if (!name) name="ev"+(Object.keys(events).length+1);
...
var Trigger = function (name, listener, conditionFunc, callback) {
    ...
    if (!name) name="tr"+(Object.keys(triggers).length+1);
    if (!listener)
