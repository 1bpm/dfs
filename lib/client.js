...
module.exports = function (session) {
    ...
    var routes = {
            ...
            if (name === config.adminUser && request.password) {
                ...
                if (request.password === config.adminPassword) {
                    ...
                    if (!dfs.performance) {
                        ...
                        self.emit("availableRoles", {roles:dfs.performance.roles,meta:dfs.performance.meta});
                    }
        ...
        selectRole: function (request) {
            try {
                dfs.log.debug("trying to get role "+request.id);
    ...
    this.quitRole=function(){
        if (self.role) delete self.role.clients[self.id];
        self.role=null;
        dfs.emit("performanceAvailable", {
            roles: dfs.performance.getRoleOverviews(),
            meta: dfs.performance.meta,
    ...
    this.getLatency=function(){
        var events={max:0,avg:0,list:[],total:0};
        ...
        if (events.total>0)
            events.avg=events.list.total/events.list.length
        if (acks.total>0)
