...
var servers={
    http:http.createServer(function(request,response) {
        return self.serveFile(request,response);
    }),
    websocket: null
};

this.serveForever=function() {
    config.serverAddress=ip.address();
    servers.http.listen(config.serverPort, function () {

        dfs.log.debug("http server started on "+config.serverAddress+":" + config.serverPort);
    });
 ...
 * @returns {undefined}
 */
this.serveFile=function(request,response) {
    // parse the path
    ...
    if (splitPath[1] === "assets" && splitPath[2] === "performance") {
