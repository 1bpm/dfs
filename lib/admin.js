...
module.exports = function (emitter,superClient) {
    ...
    var routes = {
        ...
        loadPackage: function (data) {
            var pkg = new PackageParser(packages[data.packageID]);
            var err = pkg.error;
            dfs.log.debug("package loaded");
            if (err.length>0) {
                self.emit("parseErrors",{errors:err});
            } else {
                self.client.emit("log",{level:"notify",text:"performance loaded"});
                delete dfs.performance;
                dfs.eachClient(function(client){
                    client.role=null;
                });
                dfs.performance=pkg.performance;
                self.performanceLoaded();
            }
        },
